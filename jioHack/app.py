...
def bot_page():
	...
	return render_template('bot.html',data=[config.topics.keys(),bot_name])

@app.route('/bot/response',methods=['GET','POST'])
def return_response():
	query=request.form['text']
	source=request.form['source']
	optionTuple=config.topics.keys()
	html_string_box=""
	html_string_sideMenu=""
	if source=='button':
		if 'issue resolved?' in query or "No,I don't want to" in query:
			html_string_box="<li id='bot_box' class='left clearfix'>Thank you for contacting us.</li>"
	...
	response=botBrain(query,source)
